...
def run_test() -> dict[str, int]:
    def ga_test():
        ...
        print(str(winner))
        return {'fitness': winner.get_total_value(), 'generations': population.generation_number}
    ...
    def climbing_test():
        ...
        print(str(winner))
        return {'fitness': winner.get_total_value(), 'generations': climber.total_iterations, 't_0': climber.initial_temperature}

    tests = {'GA': ga_test, 'Climbing': climbing_test}
    return tests[MODE]()  # type:ignore
...
def run_tests(num_tests: int) -> None:
    ...
    if 't_0' in tests[0]:
        average_initial_temperature = sum(
            [t['t_0'] for t in tests]) / num_tests
        print('average_initial_temperature', average_initial_temperature)
#     print("""%s
...
#               '-'*50, num_tests, str(best), best_closeness_to_optimal, best_frequency, str(worst), average_fitness, average_generations, average_closeness_to_optimal, '-'*50))
    print("""%s
    %d tests performed. Results:
    Best fitness: %s (%.2f%% of optimal)
    Average number of generations: %d
    Average fitness: %d (%.2f%% of optimal)
    %s"""
          % (
              '-'*50, num_tests, str(best), best_closeness_to_optimal, average_generations, average_fitness, average_closeness_to_optimal, '-'*50))
...
def main():
    ...
    if MODE == 'GA':
        ...
        settings = 'GA: %s, %s, %s, %s' % (
            DATASET_NAME, SELECTION_OPERATOR, CROSSOVER_OPERATOR, MUTATION_OPERATOR)
    elif FOOLISH_MODE:
        # settings = 'running FHC: %s, no change threshold=%d' % (
        #     DATASET_NAME, NO_CHANGE_THRESHOLD)
        settings = 'running FHC: %s, %s' % (
            DATASET_NAME, MUTATION_OPERATOR)
    else:
        # settings = 'running SA: %s, %s, alpha=%f, i_0=%d, beta=%f, no change threshold=%d' % (
        #     DATASET_NAME, MUTATION_OPERATOR, ALPHA, I_0, BETA, NO_CHANGE_THRESHOLD)
        settings = 'running SA: %s, %s' % (
            DATASET_NAME, MUTATION_OPERATOR)
    print(settings)
    run_tests(5)
    print(settings)
